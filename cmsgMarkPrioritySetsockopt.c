...
static void __attribute__((noreturn)) cs_usage(const char *bin)
{
    ...
    printf("Options:\n"
           ...
           "\t\t-p val  Set SO_PRIORITY with given value (globally)\n"
           "");
    exit(ERN_HELP);
}
...
static void cs_parse_args(int argc, char *argv[])
{
    ...
    while ((o = getopt(argc, argv, "sm:p:")) != -1) {
        switch (o) {
        case 's':
...
int main(int argc, char *argv[])
{
    ...
    // Set socket priority using setsockopt
    if (opt.priority.ena) {
        if (setsockopt(fd, SOL_SOCKET, SO_PRIORITY, &opt.priority.val, sizeof(opt.priority.val)) < 0) {
            fprintf(stderr, "Failed to set SO_PRIORITY: %s\n", strerror(errno));
            close(fd);
            freeaddrinfo(ai);
            return ERN_SETSOCKOPT;
        }
    }
